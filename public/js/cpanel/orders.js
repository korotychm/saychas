              ...
              <div class="tbody">
                <div class="tr orders__item" v-for="(order,index) in orders">
                    ...
                    <div class="td orders__btn" v-if="order.status_id == '01'">
                      <button class="btn btn--primary" @click="saveOrder(index,'02')">Приступить к сборке<span :key="currentTime">{{ order.deadline }}</span></button>
                    </div>
      ...
      checkTime(){
        ...
        for (order of this.orders){
          if (+order.status_id == '01'){
            ...
            this.$set(this.orders[i],'deadline',deadline);
            let blabla = new Date;
